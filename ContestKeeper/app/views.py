...
from .forms import (
    ...
    AnnouncementForm, ScheduleEventForm, SubmissionForm
)
...
class SubmissionCreateEditView(RedirectToRegisterMixin, View):
    """Handles both creation and editing of a submission for a round."""
...
class SubmissionDetailView(RedirectToRegisterMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        # Access control: team members, organizer, or jury
...
class RoundSubmissionsListView(OrganizerRequiredMixin, ListView):
    """List all submissions for a round (organizer and jury only)."""
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Extend OrganizerRequiredMixin to also allow Jury
        if not request.user.is_authenticated:
            return self.handle_no_permission()
